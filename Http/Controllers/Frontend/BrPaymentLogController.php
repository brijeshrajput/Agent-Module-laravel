...
class BrPaymentLogController extends Controller
{
    private const CANCEL_ROUTE = 'landlord.agent.order.payment.cancel';
    private const SUCCESS_ROUTE = 'landlord.agent.order.payment.success';

    private float $total;
    private object $payment_details;
    protected function cancel_page()
    {
        return redirect()->route('landlord.agent.order.payment.cancel.static');
    }
    ...
    public function order_payment_form(Request $request)
    {
        ...
        if($plan->price == 0)
        {
            ...
            $purchased_packaged = ZeroPricePlanHistory::where('user_id', $client_id)->count();
        ...
        if ($request->custom_subdomain != null) {
            ...
            //todo : have to enable it
            // if (!empty(get_static_option('user_email_verify_status')) && !$client->email_verified)
            // {
            //     return back()->with(FlashMsg::explain('danger', __('Please ask your client to verify their email, Visit user dashboard for verification')));
            // }
        ...
        try {
            ...
            if (!is_null($is_tenant)) {
                $old_tenant_log = PaymentLogs::where(['user_id' => $client_id, 'tenant_id' => $is_tenant->id])->latest()->first() ?? '';

                // If Payment Renewing
                if (!empty($old_tenant_log->package_id) == $request_pack_id && !empty($old_tenant_log->user_id) && $old_tenant_log->user_id == $client_id && ($old_tenant_log->payment_status == 'complete' || $old_tenant_log->status == 'trial')) {
                ...
                $old_tenant_log = PaymentLogs::where(['user_id' => $client_id, 'tenant_id' => trim($request->custom_subdomain)])->latest()->first();
        ...
        if ($request->selected_payment_gateway === 'manual_payment')
        {
            ...
            return $this->payment_with_gateway($request->selected_payment_gateway, $request->all(), $client_id, $client);
        }
    }

    public function payment_with_gateway($payment_gateway_name, $request = [], $client_id, $client)
    {
        try {
            ...
            if (!method_exists((new PaymentGatewayCredential()), $gateway_function))
            {
                ...
                $redirect_url = $gateway->charge_customer(
                    $this->common_charge_customer_data($payment_gateway_name, $client_id, $client)
                );
    ...
    public function common_charge_customer_data($payment_gateway_name, $client_id, $client)
    {
        $email = $client->email;
        $name = $client->name;
        ...
        return [
            ...
            'ipn_url' => route('landlord.agent.client.' . strtolower($payment_gateway_name) . '.ipn', $this->payment_details['id']),
